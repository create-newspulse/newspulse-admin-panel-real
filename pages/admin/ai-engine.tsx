...
export default function AIEnginePage(): JSX.Element {
  ...
  return (
    <AdminShell>
      <div className="p-0">
        ...
        <p className="text-slate-600 dark:text-slate-300 mb-6">Paste any news content or URL and generate a fresh, publication-ready article with 5W1H and SEO fields.</p>
        ...
        <button disabled={loading} onClick={run} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded disabled:opacity-60">{loading ? 'Running…' : 'Run AI Task'}</button>
          ...
          <div className="mt-6 space-y-4">
            ...
            {Array.isArray(result.outline) && result.outline.length>0 && (
              <div className="bg-white dark:bg-slate-800 p-4 rounded shadow">
                ...
                <ul className="list-disc ml-6 space-y-1">{result.outline.map((o:string,i:number)=> <li key={i}>{o}</li>)}</ul>
              </div>
            )}
            {result.article && (
              <div className="bg-white dark:bg-slate-800 p-4 rounded shadow">
            ...
            {result.disclaimer && <div className="text-xs text-slate-500">{result.disclaimer}</div>}
          </div>
        )}
      </div>
    </AdminShell>
