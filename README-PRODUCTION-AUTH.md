...
1. Admin backend host (service exposing `/api/admin/*`): `https://your-backend-host.tld`
2. In Vercel Project → Environment Variables (Production):
	- `VITE_API_URL=https://your-backend-host.tld`  ← backend origin (no trailing `/api`)
	- `ADMIN_BACKEND_URL=https://your-backend-host.tld` (optional; used by `/admin-api/*` proxy)
...
5. Network login flow should POST directly to:
	- `https://your-backend-host.tld/api/admin/login`

### Seed Founder in Direct Mode
Run this once (PowerShell):
```powershell
$body = '{"email":"newspulse.team@gmail.com","password":"News@123","force":true}'
curl.exe -X POST "https://your-backend-host.tld/api/admin/seed-founder" -H "content-type: application/json" -d $body
```
