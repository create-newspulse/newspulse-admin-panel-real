...
// Γ£à Admin Panel Lock Protection Wrapper
function useLockdownCheck(settings) {
    const navigate = useNavigate();
    useEffect(() => {
        if (settings.lockdown) {
            toast.error('≡ƒöÆ Lockdown Mode is ON. Admin tools are restricted.');
...
const ZoneSection = ({ title, icon, children }) => {
    ...
    return (_jsxs("div", { className: "bg-slate-900 text-white mb-4 rounded-xl shadow-lg", children: [_jsxs("button", { onClick: () => setOpen(!open), className: "w-full flex justify-between items-center px-6 py-3 text-left font-bold text-lg", children: [_jsxs("div", { className: "flex items-center gap-2", children: [icon, " ", title] }), _jsx("span", { children: open ? 'Γû▓' : 'Γû╝' })] }), open && _jsx("div", { className: "p-6 border-t border-slate-700", children: children })] }));
};
...
export default function AdminControlCenter() {
    ...
    useEffect(() => {
        apiClient.get('/settings/load')
            ...
            .catch(() => {
            toast.error('ΓÜá∩╕Å Failed to load settings from database.');
    ...
    const handleSave = () => {
        toast.promise(apiClient.post('/settings/save', settings), {
            ...
            success: 'Γ£à Settings saved!',
            error: 'Γ¥î Failed to save settings.',
        });
    };
    const handleReset = () => {
        setSettings(safeDefaults);
        toast.success('ΓÖ╗∩╕Å Settings reverted to Safe Mode');
    };
    const handleExportPDF = () => {
      ...
      <h2 style="font-size:18px; margin-bottom:12px;">≡ƒº⌐ NewsPulse Admin Settings</h2>
      <ul>
        <li><strong>AI Trainer:</strong> ${settings.aiTrainer ? 'Enabled Γ£à' : 'Disabled Γ¥î'}</li>
        <li><strong>AdSense:</strong> ${settings.adsenseEnabled ? 'Enabled Γ£à' : 'Disabled Γ¥î'}</li>
        <li><strong>Voice Reader:</strong> ${settings.voiceReader ? 'Enabled Γ£à' : 'Disabled Γ¥î'}</li>
        <li><strong>Lockdown Mode:</strong> ${settings.lockdown ? 'Enabled ≡ƒöÆ' : 'Disabled ≡ƒöô'}</li>
        <li><strong>Signature Lock:</strong> ${settings.signatureLock ? 'Enabled Γ£ì∩╕Å' : 'Disabled Γ¥î'}</li>
        <li><strong>Founder Only Mode:</strong> ${settings.founderOnly ? 'Active ≡ƒææ' : 'Disabled Γ¥î'}</li>
      </ul>
