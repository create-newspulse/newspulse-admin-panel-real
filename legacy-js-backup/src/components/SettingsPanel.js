...
const settingLabels = {
    panelGuideVisible: '≡ƒôÿ Show Panel Guide',
    showLoginRecords: '≡ƒöÉ Show Login Tracker',
    showBackupPanel: '≡ƒôé Enable Backup & Recovery',
    restrictToFounder: '≡ƒ¢í∩╕Å Restrict View to Founder Only',
    showSystemHealth: '≡ƒ⌐║ Show System Health Panel',
    showTrafficAnalytics: '≡ƒôè Show Traffic Analytics',
    showAIActivityLog: '≡ƒºá Show AI Activity Log',
    showRevenuePanel: '≡ƒÆ░ Show Revenue Panel',
    enableExportPDF: '≡ƒôñ Enable Export to PDF',
    allowEditorMode: '≡ƒæÑ Allow Editor-Level View',
    enableAITrainer: '≡ƒº¼ Enable AI Trainer',
};
...
const SettingsPanel = () => {
    ...
    useEffect(() => {
        if (isEditorPreview) {
            alert('≡ƒöÆ Restricted Zone ΓÇô Admins Only');
            window.location.href = '/';
        }
    }, []);
    const handleToggle = (key) => {
        const timestamp = new Date().toISOString();
        const logEntry = `[${timestamp}] ≡ƒöº ${key} ΓåÆ ${!settings[key]}`;
    ...
    const handleProtectedToggle = (key) => {
        const confirmed = window.confirm('≡ƒöÉ Founder Reactivation Required');
        if (confirmed) {
            ...
            if (input === FOUNDER_PASS) {
                ...
                alert('Γ¥î Incorrect Founder Code.');
            }
        }
    };
    useEffect(() => {
    ...
    useEffect(() => {
        if (!settings.showRevenuePanel) {
            setAiSuggestion({
                message: '≡ƒÆ░ Revenue Panel is hidden. Turn it on to monitor ad earnings.',
    ...
    return (_jsxs("section", { className: "p-6 text-white", children: [_jsxs("div", { className: "ai-card glow-panel ai-highlight hover-glow mb-4", children: [_jsxs("div", { className: "flex justify-between items-center", children: [_jsx("h2", { className: "text-2xl font-bold", children: "\u2699\uFE0F Admin Control Center \u2013 Phase 15" }), _jsx("button", { onClick: () => {
                                    const preview = confirm('≡ƒº¬ Preview As Editor Mode?');
                                    if (preview)
