...
const AutoLockdownSwitch = () => {
    ...
    const triggerLockdown = async () => {
        ...
        try {
            ...
            if (data.success ?? true) {
                setStatus("success");
                setMessage("Γ£à Lockdown activated successfully!");
                setLockPin("");
                setConfirmed(false);
                notify.success('≡ƒÜ¿ Lockdown activated');
            }
            else {
                setStatus("error");
                setMessage(data?.error || "Γ¥î Invalid PIN or server error.");
                notify.error(data?.error || 'Γ¥î Lockdown activation failed');
            }
        }
        catch (e) {
            setStatus("error");
            setMessage("Γ¥î Server error. Please try again.");
            notify.error('Γ¥î Server error during lockdown');
        }
    };
    const triggerUnlock = async () => {
        ...
        try {
            ...
            if (data.success ?? true) {
                setStatus("success");
                setMessage("Γ£à Lockdown deactivated!");
                setUnlockPin("");
                notify.success('≡ƒöô Lockdown deactivated');
            }
            else {
                setStatus("error");
                setMessage(data?.error || "Γ¥î Invalid PIN or server error.");
                notify.error(data?.error || 'Γ¥î Unlock failed');
            }
        }
        catch (e) {
            setStatus("error");
            setMessage("Γ¥î Server error. Please try again.");
            notify.error('Γ¥î Server error during unlock');
        }
    };
    return (_jsxs("section", { className: "p-5 md:p-6 bg-red-50 dark:bg-red-900/10 border border-red-500/30 rounded-2xl shadow-inner max-h-[90vh] overflow-y-auto", children: [_jsx("p", { className: "text-green-600 dark:text-green-400 font-mono text-sm mb-2", children: "\u2705 AutoLockdownSwitch Loaded" }), _jsxs("h2", { className: "text-xl font-bold text-red-700 dark:text-red-400 mb-3 flex items-center gap-2", children: [_jsx(FaShieldAlt, { className: "text-red-500" }), "Auto Lockdown Switch"] }), _jsxs("ul", { className: "list-disc list-inside ml-3 text-sm md:text-base space-y-1 text-gray-800 dark:text-gray-200", children: [_jsx("li", { children: "\uD83C\uDF69 Disables all admin tools, pipelines, and AI triggers" }), _jsx("li", { children: "\uD83D\uDD12 Blocks new logins and freezes access until reset" }), _jsx("li", { children: "\uD83D\uDCE3 Alerts the founder immediately" })] }), _jsx("input", { type: "password", "aria-label": "Lock PIN", placeholder: "Enter Lock PIN", value: lockPin, onChange: (e) => {
                    ...
                    : "bg-red-300 cursor-not-allowed"}`, children: status === "locking" ? "Locking down..." : "≡ƒöÉ Activate Lockdown" }), _jsx("input", { type: "password", "aria-label": "Unlock PIN", placeholder: "Enter Unlock PIN", value: unlockPin, onChange: (e) => {
                    ...
                    : "bg-green-300 cursor-not-allowed"}`, children: status === "unlocking" ? "Unlocking..." : "≡ƒöô Deactivate Lockdown" }), message && (_jsx("p", { className: `font-mono mt-2 ${status === "success"
                    ? "text-green-600"
                    : status === "error"
                        ? "text-red-600"
                        : ""}`, children: message }))] }));
