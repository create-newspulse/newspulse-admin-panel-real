    ...
    const runBackup = async () => {
        ...
        try {
            ...
            if (data.success ?? true) {
                setStatus("success");
                notify.success('≡ƒÆ╛ Backup completed');
            }
            else {
                setStatus("error");
                setErrorMsg(data?.error || "Backup failed. Check server logs.");
                notify.error(data?.error || 'Γ¥î Backup failed');
            }
        }
        catch (err) {
            setStatus("error");
            setErrorMsg("Γ¥î Backup Trigger Error: " + (err?.message || "Unknown error."));
            notify.error('Γ¥î Backup trigger error');
        }
    };
