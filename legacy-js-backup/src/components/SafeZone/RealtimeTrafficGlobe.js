...
function projectOrthographic(lat, lon, R) {
...
export default function RealtimeTrafficGlobe() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const res = await fetch(`${API_BASE_PATH}/system/monitor-hub`, { credentials: 'include' });
                const ct = res.headers.get('content-type') || '';
                if (!res.ok || !/application\/json/i.test(ct))
                    throw new Error('bad');
                const data = await res.json();
                const regions = (data.topRegions || ['IN', 'US', 'AE']);
                const counts = regions.reduce((acc, code) => {
    ...
    useEffect(() => {
        ...
        points.forEach(p => {
            ...
            ctx.beginPath();
            ctx.arc(px, py, 2 + p.weight * 3, 0, Math.PI * 2);
            ctx.fill();
        });
    }, [points, rotation]);
    return (_jsxs("div", { className: "p-3", children: [_jsxs("div", { className: "flex items-center justify-between mb-2", children: [_jsx("div", { className: "text-sm font-semibold text-slate-700 dark:text-slate-200", children: "Realtime Traffic Globe" }), _jsxs("div", { className: "text-xs text-slate-500", children: ["Active Users: ", _jsx("strong", { children: activeUsers })] })] }), _jsx("div", { className: "rounded-xl overflow-hidden bg-slate-900 flex items-center justify-center", children: _jsx("canvas", { ref: canvasRef, width: size, height: size, style: { width: size, height: size } }) }), _jsx("div", { className: "mt-2 text-[11px] text-slate-500 dark:text-slate-400", children: "Lite visualization without external libraries." })] }));
}
