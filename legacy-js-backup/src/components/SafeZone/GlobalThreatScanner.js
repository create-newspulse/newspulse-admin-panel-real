...
// ≡ƒôü src/components/SafeZone/GlobalThreatScanner.tsx
...
const getStatusBadge = (condition, label) => (_jsx("span", { className: `text-xs px-2 py-0.5 rounded-full font-semibold ${condition
        ...
        : 'bg-green-100 text-green-700 dark:bg-green-800/30 dark:text-green-300'}`, children: condition ? `ΓÜá∩╕Å ${label}` : `Γ£à Secure` }));
const GlobalThreatScanner = () => {
    ...
    const fetchThreatData = async () => {
        try {
            ...
            if (typeof json.xssDetected === 'boolean' &&
                ...
                throw new Error('ΓÜá∩╕Å Unexpected response format');
            }
        }
        catch (err) {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(fetchThreatData, 900000); // ΓÅ▒∩╕Å 15 min auto-refresh
