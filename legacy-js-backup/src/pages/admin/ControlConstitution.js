...
// ≡ƒôü src/pages/admin/ControlConstitution.tsx
import { useRef } from "react";
const ControlConstitution = () => {
    const contentRef = useRef(null);
    // ≡ƒöÆ Set this to true to test red alert warning
    const constitutionBroken = false;
    const handleExportPDF = async () => {
        const element = contentRef.current;
        if (!element)
            return;
        // Γ£à dynamic import so it only loads in the browser when needed
