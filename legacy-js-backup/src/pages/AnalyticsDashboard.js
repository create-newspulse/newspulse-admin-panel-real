...
// ≡ƒôü src/pages/AnalyticsDashboard.tsx
import { useEffect, useState } from 'react';
import { useTranslate } from '../hooks/useTranslate';
import useAuthGuard from '../hooks/useAuthGuard'; // ≡ƒöÉ Guard
import api from '../utils/api'; // Γ£à Axios with token (should use baseURL: '/api')
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid, Legend, } from 'recharts';
export default function AnalyticsDashboard() {
    useAuthGuard(); // ≡ƒöÉ Redirects if not logged in
    ...
    useEffect(() => {
        const fetchAnalytics = async () => {
            try {
                // Γ£à USE this for all API calls (no localhost, no hardcoded full URLs)
                ...
                console.error('Γ¥î Failed to load analytics:', err?.message);
    ...
    const chartData = summary?.topPages.map((page) => ({
        name: page._id?.length > 20 ? page._id.slice(0, 20) + 'ΓÇª' : page._id,
        views: page.count,
    })) || [];
