...
export default function EditNews() {
        ...
        const fetchNews = async () => {
            try {
                ...
                if ((data.success && data.data) || data?.title) {
                    ...
                    setError('ΓÜá∩╕Å News not found');
                }
            }
            catch (err) {
                console.warn('Direct fetch failed, attempting fallback list lookupΓÇª');
                try {
                    ...
                    console.error(e);
                    setError('Γ¥î Failed to load news');
                }
            }
            finally {
                setLoading(false);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!form.title || !form.content || !form.category || !form.language) {
            alert('ΓÜá∩╕Å All fields are required');
            return;
        }
        try {
            ...
            if (result.success) {
                toast.success('Γ£à News updated');
                navigate('/manage-news');
            }
            else {
                toast.error('Γ¥î Update failed');
            }
        }
        catch (err) {
            console.error(err);
            toast.error(`Γ¥î Server error: ${err?.message || 'unknown'}`);
        }
    };
    const handleAISummary = async () => {
        try {
            ...
            if (summary) {
                ...
                toast.success('≡ƒºá Summary updated!');
            }
        }
        catch (err) {
            toast.error(`Γ¥î AI summarizer failed: ${err?.message || 'unknown'}`);
            console.error(err);
        }
    };
    // Checklist handlers
    ...
    const saveChecklist = async () => {
        ...
        try {
            ...
            if (w) {
                setWorkflow(w);
                toast.success('Γ£à Checklist saved');
            }
            else {
                toast.success('Γ£à Checklist updated');
            }
        }
        catch (err) {
            toast.error(`Γ¥î Save failed: ${err?.message || 'unknown'}`);
        }
    };
    const doTransition = async (action) => {
        ...
        try {
            ...
            if (w)
                setWorkflow(w);
            toast.success(`Γ£à ${action} complete`);
        }
        catch (err) {
