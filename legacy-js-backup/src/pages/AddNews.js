...
const AddNews = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!title.trim() || !content.trim() || !category.trim()) {
            toast.error('Γ¥î Please fill in all required fields.');
            return;
        }
        setLoading(true);
        try {
            ...
            toast.success('Γ£à News article added successfully!');
            ...
            toast.error(`Γ¥î Failed to add news: ${error.message || 'An unknown error occurred.'}`);
        }
        finally {
            setLoading(false);
        }
    ...
    return (
    // Γ£à CRITICAL UPDATE HERE: Added bg-white and dark:bg-slate-800 to this wrapper div
