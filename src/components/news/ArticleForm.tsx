...
export const ArticleForm: React.FC<ArticleFormProps> = ({
  ...
  const coverUploadStatusText = (() => {
    if (mediaStatusQuery.isLoading) return 'Checking upload availabilityGÇª';
  ...
  function generateSummary(titleText: string, contentText: string): string {
    ...
    // If there's no content yet, synthesize a 2GÇô3 line summary from the title in chosen language
    if (!c && t) {
      const base = t.replace(/\s+/g, ' ');
      if (language === 'gu') {
        // Gujarati localized template
        const s1 = `a¬å a¬¦a½ça¬û ${base} a¬¦a¬+a¬+a¬»a¬¿a½ç a¬åa¬¦a¬¦a½Ç a¬¦a½ç a¬¢a½ç.`;
        const s2 = `a¬ña½ça¬«a¬+a¬é a¬«a½üa¬ûa½ìa¬» a¬ña¬Ña½ìa¬»a½ï, a¬¬a¬¦a¬+a¬+a½ìa¬Ña¬+a¬ña¬+ a¬àa¬¿a½ç a¬åa¬ùa¬¦ a¬¦a½üa¬é a¬ºa½ìa¬»a¬+a¬¿a¬«a¬+a¬é a¬¦a¬+a¬ûa¬¦a½üa¬é a¬ña½ç a¬£a¬úa¬+a¬¦a½ç a¬¢a½ç.`;
        const s3 = `a¬åa¬¿a½ï a¬àa¬¦a½ìa¬Ñ a¬àa¬¿a½ç a¬«a¬¦a¬ña½ìa¬¦ a¬+a¬«a¬£a¬¦a¬+ a¬«a¬+a¬ƒa½ç a¬åa¬ùa¬¦ a¬¦a¬+a¬éa¬Üa½ï.`;
        const combinedGu = `${s1} ${s2} ${s3}`;
        return combinedGu.length > 300 ? combinedGu.slice(0, 297) + 'GÇª' : combinedGu;
      } else if (language === 'hi') {
        // Hindi template (simple neutral tone)
        const s1 = `añ»añ¦ añ¦aÑçañû ${base} añ¦añ+añ+añ» añòaÑï añòañ¦añ¦ añòañ¦aññañ+ añ¦aÑê.`;
        const s2 = `añçañ+añ«aÑçañé añ«aÑüañûaÑìañ» aññañÑaÑìañ», añ+añéañªañ¦aÑìañ¡ añöañ¦ añåañùaÑç añòaÑìañ»añ+ añªaÑçañûañ¿añ+ añ¦aÑê añ¼aññañ+añ»añ+ añùañ»añ+ añ¦aÑê.`;
        const s3 = `añçañ+añòaÑç añàañ¦aÑìañÑ añöañ¦ añ«añ¦aññaÑìañ¦ añòaÑï añ+añ«añ¥añ¿aÑç añòaÑç añ¦añ+añÅ añåañùaÑç añ¬añóañ+aÑçañé.`;
        const combinedHi = `${s1} ${s2} ${s3}`;
        return combinedHi.length > 300 ? combinedHi.slice(0, 297) + 'GÇª' : combinedHi;
      } else {
        ...
        return combined.length > 300 ? combined.slice(0, 297) + 'GÇª' : combined;
      }
    }
    ...
    return summary.length > 300 ? summary.slice(0,297) + 'GÇª' : summary;
  }
  ...
  function trimSummaryTo160(){ setSummary(s => (s.length <= 160 ? s : s.slice(0,160).replace(/\s+\S*$/, '') + 'GÇª')); }
  ...
  const saveStatusText = useMemo(() => {
    if (isPublishing) return 'PublishingGÇª';
    if (isSaving || mutation.isPending) return 'SavingGÇª';
    if (autosaveFailed) return 'Autosave failed GÇö retry';
  ...
  const seoBadgeText = checks.seo ? 'preview' : 'GÇö';
  const readabilityBadgeText = typeof readabilityGrade === 'number' ? String(readabilityGrade) : 'GÇö';
  ...
  const accordionItems: AccordionItem[] = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        badge: <span className={ptiOk ? badgeClsOk : badgeClsWarn}>{ptiOk ? 'G£à' : 'GÜán+Å'}</span>,
        defaultOpen: false,
        forceOpenWhen: hasPtiIssues,
        children: (
          <div className="space-y-2">
            <button type="button" onClick={runPti} className="btn-secondary text-xs">Run PTI Check</button>
            <div className="text-sm">Status: {ptiStatus === 'compliant' ? 'G£à Compliant' : 'GÜán+Å Needs Review'}</div>
            {ptiReasons.map(r=> <div key={r} className="text-xs text-red-600">GÇó {r}</div>)}
          </div>
        ),
      },
      {
        ...
        badge: <span className={languageOk ? badgeClsOk : badgeClsWarn}>{languageOk ? 'G£à' : 'GÜán+Å'}</span>,
        defaultOpen: false,
            ...
            <div className="space-y-2">
              {(['en','hi','gu'] as const).map(l => (
                <div key={l} className="text-xs">
                  <strong>{l.toUpperCase()}:</strong> {(langIssues[l]||[]).length === 0 ? 'No issues G£à' : `${(langIssues[l]||[]).length} issues`}
                  {(langIssues[l]||[]).map((iss,i)=>(<div key={i} className="text-red-600">GÇó {iss.message}</div>))}
                </div>
              ))}
            </div>
          </div>
      ...
      },
      {
        ...
        children: (
          <div className="space-y-2">
            ...
            <div className="text-xs">Grade: {readabilityGrade ?? 'GÇö'}</div>
            <div className="text-xs">Reading Time: {readingSeconds ? `${readingSeconds}s` : 'GÇö'}</div>
          </div>
        ),
      },
    ];
  ...
  async function beginPublishFlow() {
    ...
    if (needsPtiConfirm) {
      setConfirmState({
        title: 'PTI review not cleared GÇö publish anyway?',
        ...
        onConfirm: () => {
          ...
          if (needsLangConfirm) {
            setConfirmState({
              title: 'Language issues detected GÇö publish anyway?',
          ...
          <div className="card p-4 space-y-4">
            ...
            <div>
              ...
              <div className="mt-2 text-xs">
                ...
                {slugCheck.status === 'checking' && <span className="text-slate-600">CheckingGÇª</span>}
                {slugCheck.status === 'available' && <span className="text-green-700">G£à Slug available</span>}
            ...
            <div>
              ...
              {suggestions && (
                <div className="mt-2 flex items-center gap-2 flex-wrap">
                  ...
                  <span className={`text-[11px] ${summary.length>=120 && summary.length<=180 ? 'text-green-600' : 'text-amber-600'}`}>{summary.length} chars (aim 120GÇô180)</span>
                </div>
              )}
            ...
            <div>
              ...
              <RichTextEditor
                value={content}
                onChange={setContent}
                placeholder="Write article contentGÇª"
              />
            </div>
          </div>
        </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <select value={category} onChange={e=> setCategory(e.target.value)} className="w-full border px-2 py-2 rounded">
                  <option value="" disabled>Select categoryGÇª</option>
              ...
              {category === 'youth-pulse' && (
                <div>
                  ...
                  <select value={youthPulseTrack} onChange={e=> setYouthPulseTrack((e.target.value || '') as YouthPulseTrack | '')} className="w-full border px-2 py-2 rounded">
                    <option value="">Select Youth Pulse trackGÇª</option>
                ...
                {!locationTagsCollapsed && (
                  <>
                    <input
                      ...
                      placeholder="Search districts / citiesGÇª"
                    />
                  ...
                  <div className="mt-3 rounded border border-slate-200 bg-white p-3">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="text-[11px] text-slate-600">
                        {groupStatus ? `Group Status: ${groupStatus}` : 'Group Status: GÇö'}
                      </div>
                    </div>
                    ...
                    {translationGroupQuery.isFetching ? (
                      <div className="mt-2 text-[11px] text-slate-500">Loading variantsGÇª</div>
                    ) : null}
              ...
              <div className="pt-3 border-t border-slate-200">
                ...
                />
                {isUploadingCover && <div className="mt-1 text-[11px] text-slate-500">UploadingGÇª</div>}
      ...
      {publishSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-4 shadow-lg">
            <div className="text-sm font-semibold text-slate-900">
              {publishSuccess.syncResults ? 'Published and synced' : 'Published G£à (Translations pending)'}
            </div>
        ...
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            {(effectiveId && translationStatus === 'failed') && (
              ...
              >
                {retryTranslationMutation.isPending ? 'RetryingGÇª' : 'Retry Translation'}
              </button>
            )}
            ...
            <span className="inline-flex" title={publishTooltip}>
              ...
              >
                {isPublishing ? 'PublishingGÇª' : 'Publish'}
              </button>
            </span>
          </div>
        </div>
