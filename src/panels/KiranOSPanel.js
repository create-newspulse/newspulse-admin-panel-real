    ...
    const handleAskKiranOS = async () => {
        ...
        while (attempt <= maxRetries) {
            try {
                ...
                setChatResponse(res.data?.answer ?? res.data?.reply ?? '🤖 No response.');
                setIsAsking(false);
                return;
            }
            catch (err) {
                ...
                if (isBusy && attempt < maxRetries) {
                    ...
                    setChatResponse(`⏳ KiranOS is busy. Retrying in ${Math.ceil(delay / 1000)}s…`);
        ...
        if (status === 429 || /rate/i.test(msg)) {
            setChatResponse('⏳ KiranOS is busy. Please try again in a few seconds.');
        }
        else {
