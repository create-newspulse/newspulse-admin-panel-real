...
export default function AITrainerPanel() {
  ...
  console.log('📡 Fetching: AI training info');
      const res = await apiClient.get(AI_TRAINING_INFO_URL.replace(/^.*\/system\//, '/system/ai-training-info'));
  ...
  const handleRetrainNow = async () => {
    ...
    try {
      ...
      const res = await apiClient.put(AI_TRAINING_INFO_URL.replace(/^.*\/system\//, '/system/ai-training-info'), updatedData);
      if (res.data?.success) {
