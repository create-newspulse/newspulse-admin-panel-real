...
export async function getSettings(): Promise<SiteSettings> {
  try {
    ...
    let raw = await adminJson<unknown>("api/admin/settings", { cache: "no-store" });
    if (!raw) {
      if (import.meta.env.DEV) console.warn("[adminSettingsApi] /api/admin/settings empty; falling back to api/admin/settings/load");
      raw = await adminJson<unknown>("api/admin/settings/load", { cache: "no-store" });
    }
...
export async function putSettings(
  ...
  let raw: unknown;
  try {
    raw = await adminJson<unknown>("api/admin/settings", {
      method: "PUT",
      headers: {
...
export function exportSettingsBlob(): Promise<Blob> {
  ...
  return adminFetch("api/admin/settings/export").then(async (r) => {
