...
export function checkCsrf(req: VercelRequest, res: VercelResponse): boolean {
  ...
  const header = (req.headers['x-csrf'] as string) || '';
  if (mockOn) return true; // bypass in mock
  if (!header) {
...
export function requireReauth(req: VercelRequest, res: VercelResponse): boolean {
  // TODO: verify fresh password/JWT re-auth
  const mockOn = String(process.env.VITE_USE_MOCK || process.env.USE_MOCK || '').toLowerCase() === 'true';
  const header = (req.headers['x-reauth'] as string) || '';
  if (mockOn) return true; // bypass in mock
  if (!header) {
...
export function signUrl(path: string, secret: string, expiresInSec = 600): string {
