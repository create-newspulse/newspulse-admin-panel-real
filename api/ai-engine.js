...
async function callGemini(model, prompt, apiKey) {
    ...
    if (!key)
        throw new Error('Gemini API key missing (set GOOGLE_API_KEY or GEMINI_API_KEY)');
...
const REQUIRED_ENV = ['ADMIN_JWT_SECRET'];
export default async function handler(req, res) {
    try {
        ...
        const { provider = 'auto', model = '', language = 'English', taskType = 'Rewrite', founderCommand = '', sourceText = '', url = '', } = body || {};
        ...
        if (chosenProvider === 'openai')
            raw = await callOpenAI(model, prompt, process.env.OPENAI_API_KEY);
        else if (chosenProvider === 'anthropic')
            raw = await callAnthropic(model, prompt, process.env.ANTHROPIC_API_KEY);
        else if (chosenProvider === 'gemini')
            raw = await callGemini(model, prompt, process.env.GOOGLE_API_KEY || process.env.GEMINI_API_KEY);
        else
