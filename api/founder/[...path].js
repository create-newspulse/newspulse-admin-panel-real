...
function bad(res, message = 'Bad Request', code = 400) { return res.status(code).json({ ok: false, error: message }); }
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'POST' && !checkCsrf(req, res))
            return;
        switch (segs[0]) {
            case 'profile':
                if (method !== 'GET')
            ...
            case '2fa':
                ...
                if (segs[1] === 'disable' && method === 'POST') {
                    logFounderAction(ctx, '2FA_DISABLE');
                    return ok(res);
                }
            ...
            case 'ai':
                ...
                if (!rateLimit(`${ctx.ip}:ai:${segs[1] || 'root'}`, 10, 60_000))
                ...
                if (segs[1] === 'toggles' && method === 'POST') {
                    logFounderAction(ctx, 'AI_TOGGLES', req.body);
                    return ok(res);
                }
                if (segs[1] === 'command' && method === 'POST') {
                    if (!requireReauth(req, res))
                        return; // require re-auth for commands
            ...
            case 'system':
                if (segs[1] === 'lockdown' && method === 'POST') {
                    ...
                    if (!confirm)
                        return bad(res, 'Confirmation required', 400);
                    if (!requireReauth(req, res))
                        return;
                    if (!rateLimit(`${ctx.ip}:lockdown`, 3, 60_000))
                        return bad(res, 'Too Many Requests', 429);
                ...
                if (segs[1] === 'reactivate' && method === 'POST') {
                    if (!requireReauth(req, res))
                        return;
                    if (!rateLimit(`${ctx.ip}:reactivate`, 3, 60_000))
                        return bad(res, 'Too Many Requests', 429);
            ...
            case 'backup':
                ...
                if (segs[1] === 'trigger' && method === 'POST') {
                    logFounderAction(ctx, 'BACKUP_TRIGGER');
                    return ok(res, { jobId: 'job_123' });
                }
                return bad(res, 'Not Found', 404);
            case 'legal':
                if (segs[1] === 'certificates' && method === 'GET') {
                    return ok(res, { items: [{ id: 'cert-1', name: 'Ownership Certificate', url: '/docs/cert.pdf' }] });
                }
                if (segs[1] === 'pti-settings' && method === 'POST') {
                    logFounderAction(ctx, 'PTI_SETTINGS', req.body);
                    return ok(res);
                }
                if (segs[1] === 'successor' && method === 'POST') {
                    logFounderAction(ctx, 'SUCCESSOR_SET', req.body);
                    return ok(res);
                }
                return bad(res, 'Not Found', 404);
            case 'monetization':
                if (segs[1] === 'summary' && method === 'GET') {
                    return ok(res, { adsense: 'active', affiliate: 'ok', sponsor: 'ok' });
                }
                if (segs[1] === 'earnings' && method === 'GET') {
                    return ok(res, { daily: 42.7, monthly: 1234.5, range: req.query.range || 'monthly' });
                }
                if (segs[1] === 'lock' && method === 'POST') {
                    logFounderAction(ctx, 'REVENUE_LOCK', req.body);
                    return ok(res, { locked: !!req.body?.enabled });
                }
                if (segs[1] === 'export' && method === 'GET') {
                    return ok(res, { url: 'https://example.com/revenue.csv' });
                }
                return bad(res, 'Not Found', 404);
            case 'analytics':
                if (segs[1] === 'traffic-growth' && method === 'GET') {
                    return ok(res, { points: [10, 20, 30, 25, 40] });
                }
                if (segs[1] === 'health-summary' && method === 'GET') {
                    return ok(res, { uptime: 99.95, incidents: 0 });
                }
                if (segs[1] === 'heatmap' && method === 'GET') {
                    return ok(res, { matrix: [[1, 2, 3], [3, 2, 1]] });
                }
                if (segs[1] === 'insights' && method === 'GET') {
                    return ok(res, { items: ['Boost homepage speed', 'Increase AI fact-check threshold'] });
                }
                return bad(res, 'Not Found', 404);
            case 'insights':
                if (method === 'GET') {
            ...
            case 'security':
                if (segs[1] === 'emergency' && method === 'POST') {
                    ...
                    if (!requireReauth(req, res))
                        return;
                    if (!rateLimit(`${ctx.ip}:emergency`, 2, 60_000))
                        return bad(res, 'Too Many Requests', 429);
                    const mockOn = String(process.env.VITE_USE_MOCK || process.env.USE_MOCK || '').toLowerCase() === 'true';
                    if (!mockOn) {
                ...
                if (segs[1] === 'logs' && method === 'GET') {
                    return ok(res, { items: [{ ts: new Date().toISOString(), level: 'warn', message: 'Suspicious login blocked' }] });
                }
                if (segs[1] === 'autoprotection' && method === 'POST') {
                    logFounderAction(ctx, 'AUTOPROTECTION', req.body);
                    return ok(res);
                }
                return bad(res, 'Not Found', 404);
            default:
