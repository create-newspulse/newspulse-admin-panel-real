...
$FrontendPort = 5173
if ($env:NP_ADMIN_PORT -and ($env:NP_ADMIN_PORT -match '^\d+$')) { $FrontendPort = [int]$env:NP_ADMIN_PORT }

Write-Host "Stopping any running dev servers on frontend port $FrontendPort and the demo backend port..." -ForegroundColor Yellow
...
$DefaultLocalBackend = 'http://localhost:5000'

# Default: use NP_REAL_BACKEND if provided
$RealBackend = $env:NP_REAL_BACKEND
if (-not $RealBackend) { $RealBackend = $DefaultLocalBackend }
...
if ($Demo) {
    ...
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$ScriptDir\admin-backend'; Write-Host 'Demo Backend Server' -ForegroundColor Green; `$env:PORT='$DemoPort'; npm run dev:demo"
...
Write-Host "Frontend proxy target: $target" -ForegroundColor Cyan
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$ScriptDir'; Write-Host 'Frontend Server' -ForegroundColor Green; `$env:NP_ADMIN_PORT='$FrontendPort'; `$env:VITE_ADMIN_API_TARGET='$target'; `$env:VITE_DEV_PROXY_TARGET='$target'; `$env:VITE_DEMO_MODE='false'; `$env:VITE_USE_MOCK='false'; npm run dev"
...
$frontend = netstat -ano | findstr ":$FrontendPort.*LISTENING"
if ($frontend) {
...
Write-Host "Open your browser to: http://localhost:$FrontendPort" -ForegroundColor Yellow
try {
